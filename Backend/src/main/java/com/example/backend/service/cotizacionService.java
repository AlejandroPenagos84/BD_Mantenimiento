...
import com.example.backend.DAOs.implemetaciones.cotizacionDAOImpl;
...
public class cotizacionService {
    ...
    public List<cotizacionDTO> getAll(String user, String password) {
        conexionBD conexionBD = new conexionBD(user, password);

        try (Connection conn = conexionBD.obtenerConexion()) {
            cotizacionDAO dao = new cotizacionDAOImpl(conn);
            return dao.findAll();
    ...
    public Optional<cotizacionDTO> getById(String user, String password, String id) {
        conexionBD conexionBD = new conexionBD(user, password);
    ...
    public void save(String user, String password, cotizacionDTO entity) {
        conexionBD conexionBD = new conexionBD(user, password);
    ...
    public void update(String user, String password, String id, cotizacionDTO entity) {
        conexionBD conexionBD = new conexionBD(user, password);

        try (Connection conn = conexionBD.obtenerConexion()) {
            cotizacionDAO dao = new cotizacionDAOImpl(conn);
            if (dao.findById(id) == null) {
                throw new RuntimeException("cotizacion con ID " + id + " no encontrado.");
            }
